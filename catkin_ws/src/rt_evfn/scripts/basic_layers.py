...
def build_resnet_block(input_res, channelsout, is_training, data_format,
                       do_batch_norm=False, name=None, conv_name1=None, conv_name2=None):
    inputs = tf.identity(input_res)
    out_res_1 = general_conv2d(input_res,
                               name=name+'_res1',
                               conv_name=conv_name1,
...
def general_conv2d(conv, name=None, conv_name=None, channelsout=64, ksize=3, strides=2, init_factor=0.1,
                   padding='SAME', do_batch_norm=False, activation=tf.nn.relu,
                   is_training=True, data_format=None):
    
    conv = tf.keras.layers.Conv2D(channelsout,
                            ksize,
                            strides=strides,
                            padding=padding,
                            activation=activation,
                            kernel_initializer=\
                            tf.keras.initializers.VarianceScaling(scale=init_factor),
                            bias_initializer=tf.constant_initializer(0.0),
                            data_format=data_format,
                            name=conv_name)(conv)

    if do_batch_norm:
        conv = tf.keras.layers.BatchNormalization(axis=1 if data_format=='channels_first' else -1,
                                             epsilon=1e-5,
                                             gamma_initializer=tf.constant_initializer([0.01]),
                                             name=name+'_bn',
                                             trainable=is_training)(conv)
    return conv
...
"""
def upsample_conv2d(conv, name=None, conv_name=None, channelsout=64, ksize=3, init_factor=0.1,
    ...
    conv = general_conv2d(conv, name=name, conv_name=conv_name, channelsout=channelsout, ksize=ksize, strides=1,
                          do_batch_norm=do_batch_norm, padding='VALID', init_factor=init_factor,
                          is_training=is_training, data_format=data_format)
    
    return conv

def predict_flow(conv, name=None, conv_name=None, channelsout=2, ksize=1, strides=1,
