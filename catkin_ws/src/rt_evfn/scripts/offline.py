...
class event_image:
    ...
    def ImageStream(self):
        ...
        while len(self.events) > 0:
            ...
            t_plus = t0 + self.fps
            print(t0, t_plus, len(self.events))
            events = filter(lambda x: ((x.ts).to_sec() < t_plus), self.events)
            if (len(events) > 0):
                self.events = np.delete(self.events, range(len(events)), axis=0)
            ...
            for event in events:
                ...
                t = (event.ts - self.start_t).to_sec() - (t0 - self.start_t.to_sec())
...
def realtime():
    ...
    events.Config(fps, nframe, [256, 256, 256, 256]) # x:192-447, y:112-367
    ...
    with tf.Session(config=config) as sess:
        ...
        #s = event_images.shape
        #print(s, event_count)
        #f = open("event_image.txt","w")
        
        #f.write(str(s) + "\n\n\n")
        #for i in range(10, 11):
        #    for j in range(s[1]):
        #        for k in range(s[2]):
        #            out = np.float64(event_images[i, j, k, :]).astype(str)
        #            out = str(out)
        #            f.write(out)
        #        f.write("\n")
        #    f.write("\n\n")
